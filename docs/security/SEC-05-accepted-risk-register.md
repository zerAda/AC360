...
| **Document ID Positive-Format Floor** (R1) | Security / Tech Debt | **mitigated** *(plancher de longueur)* | `_validate_sharepoint_doc_id` rejette les id `< _DOCID_MIN_LEN` (10) : un id trop court (« ! ») est refusé 400 AVANT tout quota/orchestration, validation déplacée avant le quota (CB-02, commit `0f94ac8`). Résiduel accepté-différé : un id de 10+ caractères au charset valide mais inexistant peut encore démarrer une orchestration vouée à l'échec — borné à ≤10/user/h (quota) + téléchargement as-user gardé (taille/ext/timeout). Vérification de format positive complète (alphabet drive-item) différée. | SECURITY_REDTEAM_STAGING_2026-06 (R1) ; commit `0f94ac8` |
