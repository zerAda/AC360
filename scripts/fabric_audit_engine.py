"""fabric_audit_engine.py — Moteur d'audit AC360 robuste et typé.

Import-safe : n'importe AUCUN SDK cloud, ne lit AUCUNE donnée. Pur calcul.

Rôle :
  * aliasing des libellés OCR arbitraires (Azure Document Intelligence renvoie
    "Raison sociale", "Plafond hospi.", etc.) vers des champs canoniques ;
  * normalisation des montants / dates / noms / numéros de contrat ;
  * comparaison TYPÉE document vs référence Fabric avec statut
...
Le résultat respecte schemas/audit_result.schema.json.
"""
...
def alias_field(label: Any) -> Optional[str]:
    ...
    contenir l'un des alias normalisés (ordre = priorité). On teste les alias
    les plus spécifiques d'abord (longueur décroissante) pour éviter qu'un alias
    générique ("nom") ne capture un libellé plus précis.
    """
...
def _find_in_tables(tables: List[dict], aliases: List[str]) -> Optional[str]:
    ...
    dessous) de la cellule-libellé, en utilisant row_index/column_index."""
...
def extract_canonical_fields(ocr_result: dict) -> Dict[str, str]:
    ...
    (libellés arbitraires de Document Intelligence). Ne lève jamais."""
    out: Dict[str, str] = {}
    ...
    if "plafond_hospitalisation" not in out:
        v = _find_in_tables(ocr_result.get("tables", []), _FIELD_ALIASES["plafond_hospitalisation"])
        if v:
...
def audit(audit_input: dict) -> dict:
    ...
    audit_input respecte schemas/audit_input.schema.json. Ne lit aucune donnée
    externe : tout est fourni dans l'argument.
    """
    document = audit_input.get("document", {}) or {}
    reference = audit_input.get("reference", {}) or {}
    ...
    return {
        ...
        "meilleur_match_fabric": reference.get("nom_client"),
