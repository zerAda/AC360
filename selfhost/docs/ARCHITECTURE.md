# Architecture — Stack IA privée AC360 (Onyx + Ollama)
...
Tous les liens inter-services empruntent le réseau interne `ac360-local-ai-net`.
...
## 7. Positionnement vs AC360 (Azure / Copilot Studio)

AC360 « cloud » : intégration Microsoft 365, RAG SharePoint, Entra ID, Azure OCR.
Cette stack « locale » : même finalité RAG, mais **inférence et données sur
site**. Les deux partagent la philosophie *réponses sourcées, sécurité d'abord*.
Pour aligner l'auth, brancher Onyx sur **Entra ID (OIDC)** (SECURITY.md §6).
```
AC360 (cloud)   : Teams/Copilot → SharePoint → Entra ID → Azure
AC360 (local)   : Navigateur   → Onyx       → OpenSearch/MinIO → Ollama
